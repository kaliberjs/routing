  ...
  describe('object route conversion', () => {
    ...
    describe('child routes', () => {
      ...
      test('localized leaf', () => {
        ...
        expect(x()).toBe('a')
        expect(x.child()).toBe('a/b')
        expect(x.child.nested()).toEqual('a/b/c')
        const { y } = asRouteMap({ y: map }, 'nl')
        expect(y()).toBe('a')
        expect(y.child()).toBe('a/b')
        expect(y.child.nested()).toEqual('a/b/d')
      })
      test('localized branch', () => {
        ...
        const { x } = asRouteMap({ x: map }, 'en')
        expect(x()).toBe('a')
        expect(x.child()).toEqual('a/b')
        expect(x.child.nested()).toEqual('a/b/d')
        const { y } = asRouteMap({ y: map }, 'nl')
        expect(y()).toBe('a')
        expect(y.child()).toEqual('a/c')
        expect(y.child.nested()).toEqual('a/c/d')
      })
      test('localized root', () => {
        const map = {
          path: { en: 'a', nl: 'b' },
        ...
        const { x } = asRouteMap({ x: map }, 'en')
        expect(x()).toEqual('a')
        expect(x.child()).toEqual('a/c')
        expect(x.child.nested()).toEqual('a/c/d')
        const { y } = asRouteMap({ y: map }, 'nl')
        expect(y()).toEqual('b')
        expect(y.child()).toEqual('b/c')
        expect(y.child.nested()).toEqual('b/c/d')
      })
      test('combined dynamic reverse routing', () => {
        ...
        expect(x.child({ a: 'i', c: 'j', d: 'k' })).toEqual('i/b/j/e')
        expect(x.child.nested({ a: 'i', c: 'j', d: 'k', g: 'l' })).toEqual('i/b/j/e/l/h')
        const { y } = asRouteMap({ y: map }, 'nl')
        expect(y({ a: 'i' })).toBe('i/b')
        expect(y.child({ a: 'i', c: 'j', d: 'k' })).toEqual('i/b/k/f')
        expect(y.child.nested({ a: 'i', c: 'j', d: 'k', g: 'l' })).toEqual('i/b/k/f/l/h')
      })
    })
  })
  ...
  describe('pick', () => {
    ...
    describe('found', () => {
      ...
      test('wildcard', () => {
        ...
        expect(pick('/a', [map, asTuple])).toEqual([{ '*': '' }, map.a])
      ...
      test('static before wildcard nested', () => {
        const map = asRouteMap({ a: '*', b: 'b/*', c: 'b', d: 'b/b' })
        expect(pick('/a', [map, asTuple])).toEqual([{ '*': 'a' }, map.a])
      ...
      test('dynamic before wildcard nested', () => {
        ...
        expect(pick('/a', [map, asTuple])).toEqual([{ '*': '' }, map.b])
